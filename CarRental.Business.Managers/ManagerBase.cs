...
namespace CarRental.Business.Managers
{
    ...
    public class ManagerBase
    {
            ...
            if (!Thread.CurrentPrincipal.IsInRole(Security.CarRentalAdminRole))
            {
                if (_AuthorizationAccount != null)
                {
                    if (_LoginName != string.Empty && entity.OwnerAccountId != _AuthorizationAccount.Id)
                    {
                        ...
                        throw new FaultException("Attempt to access a secure record with improper user authorization validation.");
                    }
                }
            }
        }

        protected T ExecuteFaultHandleOperation<T>(Func<T> codeExecute)
        {
            try
            {
                return codeExecute.Invoke();
            }
        ...
        protected void ExecuteFaultHandleOperation(Action codetoExecute)
        {
            try
            {
