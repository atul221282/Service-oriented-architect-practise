            ...
            if (reservations != null)
            {
                foreach (Reservation reservation in reservations)
                {
                    using (TransactionScope scope = new TransactionScope())
                    {
                        try
                        {
                            rentalManager.CancelReservation((int)reservation.Id.Value);
                            Console.WriteLine("Canceling reservation '{0}'.", reservation.Id);

                            scope.Complete();
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine("There was an exception when attempting to cancel reservation '{0}'.", reservation.Id.Value);
                        }
                    }
                }
            }
